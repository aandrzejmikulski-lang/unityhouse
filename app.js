...
async function loadPendingUsers() {
  const { data } = await client.from("profiles").select("id, full_name").eq("approved", false).eq("role", "user");
  pendingUsersList.innerHTML = "";
  (data || []).forEach((u) => {
    const div = document.createElement("div");
    div.innerHTML = `<strong>${u.full_name}</strong><br><button class="btn-approve" data-id="${u.id}">Zatwierdź</button><hr>`;
